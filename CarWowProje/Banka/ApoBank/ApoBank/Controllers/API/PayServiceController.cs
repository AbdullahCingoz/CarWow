...
namespace ApoBank.Controllers.API
{
    ...
    {
        ApoBankDbEntities1 db = new ApoBankDbEntities1();
        public IHttpActionResult PostPayService(string MerchantCode, string MerchantPassword, string CardNo, string ExpM, string ExpY, string CVV, decimal Price)
        {
            ...
            if (merchantCount != 0)
            {
                ...
                int cardControl = db.MusteriHesap.Count(x => x.KartNumarasi == CardNo);
                if (cardControl != 0)
                {
                    MusteriHesap mh = db.MusteriHesap.FirstOrDefault(x => x.KartNumarasi == CardNo);
        ...
        public IHttpActionResult PostPayService(PayServiceModel model)
        {
            ...
            if (merchantCount != 0)
            {
                ...
                int cardControl = db.MusteriHesap.Count(x => x.KartNumarasi == model.CardNo);
                if (cardControl != 0)
                {
                    MusteriHesap mh = db.MusteriHesap.FirstOrDefault(x => x.KartNumarasi == model.CardNo);
